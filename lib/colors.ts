...
function convertToHSL(colors: string[]): string[]{
  ...
  for(const color of colors){
    ...
    if(max!=min){
      ...
      h = max === rNormalized ? (gNormalized - bNormalized) / d + (gNormalized < bNormalized ? 6 : 0) : max === gNormalized ? (bNormalized - rNormalized) / d + 2 : (rNormalized - gNormalized) / d + 4;
...
function generateColors(color1: string, color2: string): string[]{
  ...
  [color3,color4,color5,color6,color7] = [color3,color4,color5,color6,color7].map((color) => {
